...
def write_new_password(service_password, master_password, salt):
    ...
    with open("passwds.txt", "ab") as passwords_file:
        passwords_file.write(encrypted_password + "\n".encode("utf-8"))
    return


def read_passwords(master_password, salt):
    decrypted_passwords = []
    with open("passwds.txt", "rb") as passwords_file:
        encrypted_passwords = passwords_file.readlines()
        for password in encrypted_passwords:
            decrypted_password = str(crypto.fernet_decrypt(password,
                                                           master_password,
                                                           salt))
...
"""def delete_password(encrypted_password):
    with open("passwds.txt", """
